...
export const TwitterCard = (props: TwitterCardProps) => {
  return (
    <div className='bg-gray-200 pt-20 pb-80 flex items-center justify-center min-h-screen'>
      <div className='bg-white border-gray-200 p-4 rounded-xl border max-w-xl min-w-[480px]'>
        {props.type === 'retweet' || props.type === 'promotion' ? <div className='text-gray-500 text-sm mb-2'>{props.type === 'retweet' ? `${props.retweetedUser}さんがリツイートしました` : 'プロモーション広告'}</div> : null}
        {/* ユーザー */}
        <div className='flex items-center'>
          <img className='h-11 w-11 rounded-full' src={props.user.image} alt='' />
          <div className='ml-1.5 text-sm leading-tight'>
            <span className='text-black font-bold block '>{props.user.name}</span>
            <span className='text-gray-500 font-normal block'>{`@${props.user.accountName}`}</span>
          </div>
        </div>
        {/* 本文 */}
        <p className='text-black block text-xl leading-snug mt-3'>{props.body.text}</p>
        {props.body.image ? <img className='mt-2 rounded-2xl border border-gray-100' src={props.body.image} alt='' /> : null}
